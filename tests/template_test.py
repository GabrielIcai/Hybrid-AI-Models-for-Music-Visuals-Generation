...
def test_extract_song_name_with_real_data():
    ...
    expected_song_names = [extract_song_name(path) for path in image_paths]
...
def test_collate_fn_with_real_data():
    ...
    assert additional_features.shape == (
        ...
        3,
        12,
    )  # 5 canciones, 3 fragmentos por canción, 12 características
    assert labels.shape == (
        5,
        3,
        6,
    )  # 5 canciones, 3 fragmentos por canción, 6 etiquetas
